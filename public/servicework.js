let CacheName = "Thirukural";

let SiteName = self.location.origin;

let cacheStart = "SiteCacheVersion";
let version = 1;
        ...
        caches.match(e.request).then(response => {
            if (response) {
                ...
                let response = fetch(fetchClone).then(res => {
                    ...
                    if (e.request.url.startsWith('https://thirukuralapi.onrender.com/getThiruKural')) {
                    ...
                    else if (e.request.url.startsWith(SiteName)) {
                        const resClone = res.clone();
